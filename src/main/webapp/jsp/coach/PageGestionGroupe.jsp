				...
				<div class="players-list">
					...
					<div>
						<input type="checkbox" name="joueursIds"
							value="<%=j.getIdUtilisateur()%>" />
						<%=j.getNomUtilisateur()%>
						<%=j.getPrenomUtilisateur()%>
					</div>
					<%
					}
					} else {
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						%> <span class="empty-msg">Aucun joueur dans
							ce groupe.</span> <%
 }
 %>
					</td>
	...
	<script>
		document
				.getElementById("creerGroupeForm")
				.addEventListener(
						"submit",
						function(event) {
							const checkboxes = document
									.querySelectorAll('input[name="joueursIds"]:checked');
